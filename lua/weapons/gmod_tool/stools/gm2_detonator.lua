...
if CLIENT then
	language.Add("Tool.gm2_detonator.name", "Detonator")
	language.Add("Tool.gm2_detonator.desc", "Allows a player to place a detonator.")
	language.Add("Tool.gm2_detonator.0", "Left Click: Place, Right Click: Remove")
	language.Add("tool.gm2_detonator.time", "Duration Before Explosion:")
...
function TOOL:LeftClick(trace)
	...
	if IsValid(trace.Entity) and !trace.Entity:IsPlayer() then
		local ghostDet = ents.Create("gm2_detonator")
...
function TOOL:RightClick(trace)
	...
	if IsValid(trace.Entity) and trace.Entity:GetClass() == "gm2_detonator" then
		trace.Entity:Delete()
	end
...
function TOOL.BuildCPanel(panel)
	...
	panel:AddControl("Slider",{Label = "#tool.gm2_detonator.time", Command = "gm2_detonator_time", Min = 0, Max = 1000, type = "Float"})

...
function ENT:Draw()
    ...
    if imgui.Entity3D2D(self, Vector(-5,21.5,20), Angle(0,180,90), 0.1) then
        ...
        draw.SimpleText("Republic Weapon Crate", imgui.xFont("!Roboto@30"), -330,5, Color(226,141,152), TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
        draw.SimpleText("Weapon - " .. self:GetWeaponP(), imgui.xFont("!Roboto@20"), -330,35)
        draw.SimpleText("Quantity - " .. self:GetWeaponQuantity(), imgui.xFont("!Roboto@20"), -330,55)

        -- Main UI
        if !self:GetIsEmpty() then
            if imgui.xTextButton("Equip Weapon", "!Roboto@24", -350, 240, 600, 60, 1, Color(255,255,255), Color(226,141,152), Color(253,239,239)) then
                
            end
        else
            if imgui.xTextButton("Empty", "!Roboto@24", -350, 240, 600, 60, 1, Color(255,255,255), Color(226,141,152), Color(253,239,239)) then
    ...
    if imgui.Entity3D2D(self, Vector(50,25,30), Angle(0,90,90), 0.1) then
        ...
        if imgui.xTextButton("Admin Config", "!Roboto@24", -350, 240, 200, 60, 1, Color(255,255,255), Color(226,141,152), Color(253,239,239)) then
            net.Start("GM2:Entities:WeaponGiver:Verify")
...
local function configMenu(ent)
    ...
    panel:ClearPaint()
        ...
        :Text("Weapon Crate Config", "DermaLarge", Color(255, 255, 255), TEXT_ALIGN_LEFT, ScaleW(390), ScaleH(-240))
    ...
    CreateTab("Statistics", {
        ...
        change = function()
            ...
            weapon:SetText("Weapon: " .. ent:GetWeaponP())
            ...
            quantity:SetText("Quantity: " .. ent:GetWeaponQuantity())
    ...
    CreateTab("Config", {
        ...
        change = function()
            ...
            quantity:SetText(ent:GetWeaponQuantity())
            quantity:SetFont("menu_title")
            ...
            update.DoClick = function()
                if !tonumber(quantity:GetValue()) then
                    chat.AddText(Color(218,121,134), "[Weapon Crate]", Color(255,255,255), " Invalid quantity!")
                return end
                net.Start("GM2:Entities:WeaponGiver:Set")
                net.WriteEntity(ent)
                net.WriteString(weapon:GetValue())
                net.WriteInt(tonumber(quantity:GetValue()), 32)
                net.SendToServer()
            end
...
net.Receive("GM2:Entities:WeaponGiver:Open", function(len, ply)
